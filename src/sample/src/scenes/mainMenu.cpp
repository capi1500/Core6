...
MainMenu::MainMenu(c6::Application<ecsConfig>& app, c6::StateMachine& stateMachine) : Scene(app, stateMachine){
	...
	auto buttonFactory = c6::EntityFactoryInline<ecsConfig, const std::string&, int, c6::Callback>(
			[this](c6::EntityComponentSystem<ecsConfig>& ecs, c6::EntityComponentSystem<ecsConfig>::EntityId id, const std::string& label, int n, const c6::Callback& callback){
						...
						.attach<sf::Text>(
								c6::Function<sf::Text>(
										[&label, &n]{
											...
											text.setPosition(
													(c6::Framework::getRenderer()->getSize().x - text.getLocalBounds()
													                                                 .width) / 2,
													(c6::Framework::getRenderer()->getSize().y - text.getLocalBounds()
													                                                 .height) / 2 + n * 40
											);
											return text;
										}
				...
				b.attach<c6::Consumer<const sf::Event&>, sf::Transformable>(
						c6::Function<c6::Consumer<const sf::Event&>, sf::Transformable&>(
							[callback, this](sf::Transformable& transformable){
								return [callback, this, &transformable](const sf::Event& event){
	...
	getECS().addFromFactory<const std::string&, int, c6::Callback>(buttonFactory, "Press [1] to play", 0,
			[]{
				std::cout << "Play!\n";
			}
	);
	getECS().addFromFactory<const std::string&, int, c6::Callback>(buttonFactory, "Press [2] to exit", 1,
			[this]{
